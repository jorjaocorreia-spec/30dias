...
- **`ExpenseForm` limpa `creditCardId` ao trocar a forma de pagamento pra algo diferente de "cartão"** (`setValue('creditCardId', undefined)` no clique do botão + `null as unknown as undefined` no submit, pro `dbUpdate` realmente gravar `NULL` — mesmo padrão do `completedAt` de metas: `undefined` é descartado pelo `JSON.stringify` antes do PATCH, então nunca limpa a coluna sozinho). Sem isso, editar uma despesa de "Cartão" pra "Pix" deixava um `creditCardId` fantasma, e a despesa continuava contando na fatura/pendência do cartão
- **Todo filtro que associa despesa↔cartão deve checar `paymentMethod === 'credit_card'` junto com `creditCardId`** (nunca só `creditCardId`) — defesa contra esse tipo de dado órfão. Aplicado em `invoiceExpenses`/`cardsInUse` (`/credit-cards`), `getPendingInvoicesTotal`, `getCashBalance`, `syncCreditCardInvoices` (`useAppStore.ts`)
